...
if [ ! -f "$spotify_auth_release_dir/$spotify_auth_release_file" ]; then
  echo "Spotify auth library missing, downloading from GitHub"
  mkdir -p $spotify_auth_release_dir
  curl -L "$spotify_auth_library_location" --output "$spotify_auth_release_dir/$spotify_auth_release_file"
fi
